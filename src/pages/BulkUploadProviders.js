...
import { bulkUploadProviders } from "../services/providerService";
import {
  Container,
  Typography,
  Paper,
...
const BulkUploadProviders = () => {
  const { register, handleSubmit, formState: { errors } } = useForm();
  const navigate = useNavigate();

  const onSubmit = async (data) => {
    try {
      await bulkUploadProviders(data.file[0]);
...
export default BulkUploadProviders;